...
\toprule
\textbf{Item} & \textbf{Cost (NPR\footnote{NPR = Nepalese Rupee})} & \textbf{Remarks} \\
\midrule
...
\end{adjustbox}
\caption{Post-processing pipeline. Each step addresses a specific failure mode: thresholding gives a rough mask, top-connected filtering removes false sky blobs, CLAHE restores contrast in fog, Canny detects the true boundary, sub-pixel fitting improves precision, and smoothing removes remaining noise.}
...
\end{adjustbox}
\caption{Matching pipeline. The query profile passes through three stages of progressively finer search, then three scorers at different terrain scales are fused using Reciprocal-Rank Fusion.}
...
\includegraphics{src/images/activity_diagram.pdf}
\end{adjustbox}
\caption{System activity flow from user capture to location estimate. The feedback loop on the left rejects profiles that fail the quality gate and prompts the user to retake photos.}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{src/images/figures/fig_dataset_samples.png}
\caption{Sample images from each dataset: GeoPose3K, Google Street View, sky photos, satellite texture, and synthetic output.}
