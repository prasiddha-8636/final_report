...
\begin{itemize}
    ...
    \item Use \texttt{\textbackslash cref} for automatic `\texttt{Fig.'' prefix
\end{itemize}
...
\textbf{Rendered}: `\texttt{double quotes'' and }single quotes'.
...
\begin{itemize}
    ...
    \item \textbf{Manual numbering}: Never type `\texttt{Fig. 1'' manually---use \texttt{\textbackslash cref}
...
\begin{itemize}
    ...
    \item[$\square$] Cross-references show numbers (no }\texttt{??'')
...
\begin{itemize}
    \item \textbf{Avoid}: }\texttt{The data was analyzed'' → \textbf{Use}: }\texttt{The data were analyzed'' (data is plural)
    \item \textbf{Avoid}: }\texttt{very unique'', }\texttt{more optimal'' → \textbf{Use}: }\texttt{unique'', }\texttt{optimal'' (absolute adjectives)
    \item \textbf{Avoid}: }\texttt{In this paper'' → \textbf{Use}: }\texttt{In this report'' or }\texttt{In this work''
...
\begin{itemize}
    ...
    \item Use specific numbers rather than vague terms: }\texttt{15\% improvement'' not }`significant improvement''
