...
A \Gls{unet} with MobileNetV3-Large encoder takes a $256 \times 256$ colour image and produces a probability map of the same size, where each pixel value indicates the likelihood of being sky (high) or terrain (low).
...
Trained on synthetic scenes and GeoPose3K \cite{brejcha2019geopose3k}. The optimizer is AdamW with cosine annealing. The best checkpoint is selected by validation \Gls{iou}. Training parameters are listed in Table~\ref{tab:training_config}.
...
\paragraph{Thresholding.} Pixels with probability above 0.70 are classified as terrain, below as sky. The threshold was set by inspection on a validation set: too high misses terrain at the boundary, too low lets sky bleed into the mask.
